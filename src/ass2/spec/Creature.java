...
public class Creature {
	...
	public Creature(float x, float y, float z, float angle){
		displacement = new float[]{(float) x,(float) y,(float) z};
		facing = angle;
	}
	public void init(GL2 gl) {
		creaturePosition = new float[] { -0.2f, 0, 0.2f,// front
				0.2f, 0, 0.2f, 0.2f, 0.4f, 0.2f, -0.2f, 0.4f, 0.2f,

				0.2f, 0, 0.2f,// right
				0.2f, 0, -0.2f, 0.2f, 0.4f, -0.2f, 0.2f, 0.4f, 0.2f,

				-0.2f, 0.4f, 0.2f,// top
				0.2f, 0.4f, 0.2f, 0.2f, 0.4f, -0.2f, -0.2f, 0.4f, -0.2f,

				-0.2f, 0, 0.2f,// left
				-0.2f, 0.4f, 0.2f, -0.2f, 0.4f, -0.2f, -0.2f, 0, -0.2f,
	...
	public void draw(GL2 gl, int textureArray[], int lightMode) {
		...
		gl.glPushMatrix();
		{
			gl.glTranslated(displacement[0], displacement[1], displacement[2]);
			gl.glRotated(facing,0,1,0);
			...
			gl.glVertexPointer(3, // 3 coordinates per vertex
					...
					0); // Co-ordinates are at the start of the current array
						// buffer

			// specify the pointer to refer to the normal data in
			// GL_ARRAY_BUFFER
			...
			// specify the pointer to refer to the texture data in
			// GL_ARRAY_BUFFER
			gl.glTexCoordPointer(2, GL2.GL_FLOAT, 0,
					(creaturePosition.length + creatureNormal.length)
							* FloatBYTE);
